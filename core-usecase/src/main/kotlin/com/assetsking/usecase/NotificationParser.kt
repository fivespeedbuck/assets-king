...
object NotificationParser {
    ...
    private val amountPatterns = listOf(
        ...
        Regex("""[¥￥]\s*$NUM"""),
        // 美团退款状态真机模板：「有一笔【47.00】元的退款」。括号位于数字和“元”之间，
        // 通用 NUM元 抓不到；若继续往后找还会把“1-3个工作日”的 -3 错当金额。
        Regex("""[【\[]\s*$NUM\s*[】\]]\s*元"""),
    ...
    private val expenseKeywords = listOf(
        "支付", "付款", "消费", "扣款", "扣费", "支出", "转账给",
        "转至", "转出", "汇出",
        "已付", "成功付款", "快捷支付", "在线支付", "扫码支付", "自动扣费", "已扣费", "扣费成功",
        "向你", "转给", "付给"
    )
    ...
    private val settledMarkers = Regex(
        """消费|支出|收入|收款|到账|入账|转入|转出|转至|成功还款|成功充值|扣款成功|付款成功|支付成功|交易成功|转账成功|自动扣费|已扣费|扣费成功"""
    )
    ...
    private val hardNonTransaction = Regex(
        """账单提醒|本期应付|应还|还款日|扣款失败|待审核|拒收请回复|退订|优惠券|返现券""" +
            ...
            // 2026-09-07 真实删除样本：这些文案虽然带金额，却只是在报价、发券、预告或提醒，
            // 钱尚未实际移动。模式刻意保持具体，避免误杀真实交易后附带的普通营销尾巴。
            """|现金红包.*(?:过期|领取)|外卖红包|消费券.*(?:领取|未领取)|推荐有奖|活动邀请|秒杀""" +
            """|(?:机票|航线).*(?:价格|票价|涨价|降价|仅[¥￥])|(?:准时宝|放心吃).*生效|专属保障已生效""" +
            """|将于.*(?:扣取|扣款)|赠送费用|套餐外|账户余额已不足|余额不足提醒"""
    )
